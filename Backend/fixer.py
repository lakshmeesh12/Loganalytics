    ...
    async def receive_error(self, error_message, root_cause, remediation_steps, source, manifest_file=None, reference=None):
        ...
        if source == "windows":
            ...
            while attempt <= max_retries:
                ...
                for step in current_steps:
                    if not isinstance(step, str) or not (
                        ...
                        self.logger.warning(f"Skipped invalid Windows command: {step}")
                        continue
                    self.logger.info(f"Executing remediation step: {step} (Attempt {attempt})")
                    result = self._execute_powershell_command(step, remote=(self.target_server != "localhost"))
                    if not result["success"]:
                        self.logger.error(f"Attempt {attempt}: Failed to apply Windows remediation step: {step}, Output: {result['output']}")
                        step_success = False
                        ...
                        if attempt < max_retries:
                            ...
                            if service_name:
                                ...
                                norm_result = self._execute_powershell_command(ps_command, remote=(self.target_server != "localhost"))
                            ...
                            try:
                                ...
                                self.logger.info(f"LLM reply for fallback remediation (Attempt {attempt}): {reply_content[:200]}...")
                                ...
                                if (
                                    ...
                                    self.logger.warning(f"Invalid fallback remediation steps: {current_steps}, using default steps")
                                    current_steps = [
                                        f"cmd /c \"sc config {service_name or 'Spooler'} depend= RPCSS\"",
                                        f"Start-Service -Name {service_name or 'Spooler'}"
                                    ]
                                self.logger.info(f"Generated new remediation steps: {current_steps}")
                                break
                            except json.JSONDecodeError as json_err:
                                self.logger.error(f"JSON parsing error in LLM fallback: {json_err}, Response: {reply_content[:200]}...")
                                current_steps = [
                                    f"cmd /c \"sc config {service_name or 'Spooler'} depend= RPCSS\"",
                                    f"Start-Service -Name {service_name or 'Spooler'}"
                                ]
                                ...
                                self.logger.error(f"OpenAI API error in LLM fallback: {e}")
                                current_steps = [
                                    f"cmd /c \"sc config {service_name or 'Spooler'} depend= RPCSS\"",
                                    f"Start-Service -Name {service_name or 'Spooler'}"
                                ]
                                break
                        self.logger.info(f"Applied Windows remediation step: {step}, Output: {result['output']}")
                if step_success:
                    success = True
                    if any(step.startswith("Start-Service") for step in current_steps):
                        ...
                        if service_name_match:
                            ...
                            status_result = self._execute_powershell_command(status_cmd, remote=(self.target_server != "localhost"))
                            with open("C:/Users/Quadrant/Loganalytics/Backend/windows_errors.log", "a", encoding="utf-8") as f:
            ...
            if success and reference:
                ...
                await self._async_broadcast("FixerAgent", "remediation applied", timestamp, details, reference)
            ...
            for step in remediation_steps:
                try:
                    ...
                    self.logger.error(f"Error executing Kubernetes command: {e}")
                    step_success = False
                    with open("C:/Users/Quadrant/Loganalytics/Backend/kubernetes_errors.log", "a", encoding="utf-8") as f:
                        f.write(f"[{datetime.now()}] Remediation error for {step}:\n{str(e)}\n{'-' * 60}\n")
            if step_success:
            ...
            self.logger.warning(f"Skipping {source} remediation: Invalid source or no Snowflake connection")

    async def run_async(self):
        self.logger.info(f"FixerAgent is now running for Windows ({self.hostname}), Snowflake, and Kubernetes...")
        try:
            while True:
